...
info(V) ->
    info(V, []).
info(V, P) ->
	Time = misc:time_format(now()),
	Date = misc:date_format(now()),
	FileName = io_lib:format("~s_~s.log", [update_log, Date]),
	{ok, S} = file:open("../updatelog/" ++ FileName, [append]),
	io:format(S, "~s :" ++ V ++ "~n", [Time | P]),
	file:close(S),
    io:format(V ++ "~n", P).
